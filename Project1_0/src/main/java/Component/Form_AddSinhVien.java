 ...
 * @author admin
 */
public class Form_AddSinhVien extends javax.swing.JFrame {
    ...
    private void btndongyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btndongyActionPerformed
        if (TxtMaSinhVien.getText().isEmpty() || txtHovaTen.getText().isEmpty() || cbnNam.getSelectedIndex() == -1 || cbnNgay.getSelectedIndex() == -1 || cbnThang.getSelectedIndex() == -1
                || txtdiachi.getText().isEmpty() || txtsodienthoai.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Vui Lòng Nhập Đầy Đủ", "Nhắc Nhở", JOptionPane.INFORMATION_MESSAGE,new ImageIcon(getClass().getResource("src/main/resources/img/icons8_ask_question_20px_3.png")));

        } else {
            if (checkMaSV()) {
                JOptionPane.showMessageDialog(null, "Sinh viên Này Đã có Trong Danh Sách", "Nhắc Nhở", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource("src/main/resources/img/icons8_ask_question_20px_3.png")));

            } else {
                SinhVien Themsv = new SinhVien();
                Themsv.setMaSV(Integer.parseInt(TxtMaSinhVien.getText()));
                Themsv.setHoten(txtHovaTen.getText());
                ...
                if (RdnNam.isSelected()) {
                    Themsv.setGioitinh("Nam");
                } else {
                    Themsv.setGioitinh("Nữ");
                }
                Themsv.setDiachi(txtdiachi.getText());
                Themsv.setSoDienThoai(Integer.parseInt(txtsodienthoai.getText()));
                Themsv.setMaLop(cbnlop.getSelectedItem().toString());
                Const.session.getTransaction().begin();
                Const.session.save(Themsv);
                Const.session.getTransaction().commit();
                setOppen(!oppen);
                JOptionPane.showMessageDialog(null, "Thêm Thành Công", "Nhắc Nhở", JOptionPane.INFORMATION_MESSAGE, new ImageIcon(getClass().getResource("src/main/resources/img/icons8_ask_question_20px_3.png")));
